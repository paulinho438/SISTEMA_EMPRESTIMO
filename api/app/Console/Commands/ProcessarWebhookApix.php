...
class ProcessarWebhookApix extends Command
{
    ...
    private function processarPagamento(string $txId, float $valor, string $horario, array $data, WebhookApix $registro): bool
    {
        ...
        // Se o txId aponta para uma parcela já baixada, realoca para próximas parcelas em aberto
    ...
    /**
     * Quando um pagamento refere-se a uma parcela já baixada, aplica o valor nas próximas parcelas em aberto
     * do mesmo empréstimo (ordenadas por vencimento), abatendo excedente até zerar.
     */
    private function realocarPagamentoParaProximasParcelas(
        ...
        if ($parcelas->isEmpty()) {
            Log::channel('apix')->info('Realocação APIX: não há parcelas em aberto para aplicar pagamento', [
        ...
        if ($valorRecebido > 0 && $emprestimo && $emprestimo->banco_id && $emprestimo->company_id !== null) {
            $descricao = sprintf(
                'Pagamento PIX realocado APIX - empréstimo Nº %d do cliente %s',
                $emprestimo->id,
        ...
        Log::channel('apix')->info('Realocação APIX aplicada', [
            'identificador' => $identificadorTransacao,
            'emprestimo_id' => $emprestimoId,
    ...
    private function baixaPagamentoSaldoPendente(PagamentoSaldoPendente $pagamento, float $valor, string $horario, string $pagadorNome, string $identificadorTransacao = ''): void
    {
        ...
        if ($valorRecebido > 0 && $emprestimo->banco_id && $emprestimo->company_id !== null) {
            $descricao = sprintf(
                'Pagamento saldo pendente APIX - empréstimo Nº %d do cliente %s',
                $emprestimo->id,
                $emprestimo->client->nome_completo ?? 'N/I'
            );
            $this->registrarMovimentacao(
