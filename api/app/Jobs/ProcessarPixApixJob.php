...
class ProcessarPixApixJob implements ShouldQueue
{
    ...
    /** @var array Dados do comprovante (dados para a view, já com is_apix etc.) */
    ...
    {
        try {
            // 1. Comprovante APIX: gerar PNG e enviar por WhatsApp
            ...
            // 2. Cobranças via API APIX (parcelas, quitação, pag. mínimo, saldo pendente)
            $this->processarCobrancasApix();
            ...
            Log::channel('apix')->error('ProcessarPixApixJob: ' . $e->getMessage(), [
    ...
    /**
     * Gera cobranças PIX via API APIX para parcelas, quitação, pagamento mínimo e saldo pendente.
     */
    private function processarCobrancasApix(): void
    {
        ...
        try {
            ...
            Log::channel('apix')->error('ProcessarPixApixJob: não foi possível inicializar ApixService - ' . $e->getMessage());
            return;
        }

        foreach ($this->emprestimo->parcelas as $parcela) {
            $this->criarCobrancaApix($apixService, $parcela, $parcela->saldo ?? $parcela->valor, 'parcela_' . $parcela->id, $parcela->venc_real ? date('Y-m-d', strtotime($parcela->venc_real)) : null, function ($resp) use ($parcela) {
        ...
        if ($this->emprestimo->quitacao) {
            ...
            if ($valorQuitacao > 0) {
                $this->criarCobrancaApix($apixService, $q, $valorQuitacao, 'quitacao_' . $q->id, null, function ($resp) use ($q) {
        ...
        if ($this->emprestimo->pagamentominimo) {
            ...
            $this->criarCobrancaApix($apixService, $pm, $pm->valor, 'pagamento_minimo_' . $pm->id, null, function ($resp) use ($pm) {
        ...
        if ($this->emprestimo->pagamentosaldopendente) {
            ...
            if ($psp->valor > 0) {
                $psp->save();
                $this->criarCobrancaApix($apixService, $psp, $psp->valor, 'saldo_' . $psp->id, null, function ($resp) use ($psp) {
    ...
    /**
     * Chama ApixService criarCobranca e, em sucesso, aplica o callback para atualizar a entidade.
     */
    private function criarCobrancaApix(ApixService $apixService, $entidade, float $valor, string $referenceId, ?string $dueDate, callable $onSuccess): void
    {
        $cliente = $this->emprestimo->client;
        ...
        while ($tentativas < $maxTentativas) {
            try {
                ...
                Log::channel('apix')->error('ProcessarPixApixJob cobrança: ' . $e->getMessage(), [
...
";
    ...
    public function envioMensagemVideoYoutube($parcela): void
    {
        try {
            ...
            Log::channel('apix')->warning('ProcessarPixApixJob envio vídeo: ' . $th->getMessage());
        }
    }

    public function failed(\Exception $exception): void
    {
        Log::channel('apix')->error('ProcessarPixApixJob falhou: ' . $exception->getMessage());
    }
}
