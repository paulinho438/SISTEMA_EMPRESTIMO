...
class ProcessarPixGoldpixJob implements ShouldQueue
{
    ...
    /** @var array Dados do comprovante (dados para a view, já com is_goldpix etc.) */
    ...
    {
        try {
            // 1. Comprovante GoldPix: gerar PNG e enviar por WhatsApp
            ...
            // 2. Cobranças via API GoldPix (parcelas, quitação, pag. mínimo, saldo pendente)
            $this->processarCobrancasGoldpix();
            ...
            Log::channel('goldpix')->error('ProcessarPixGoldpixJob: ' . $e->getMessage(), [
    ...
    /**
     * Gera cobranças PIX via API GoldPix para parcelas, quitação, pagamento mínimo e saldo pendente.
     */
    private function processarCobrancasGoldpix(): void
    {
        ...
        try {
            ...
            Log::channel('goldpix')->error('ProcessarPixGoldpixJob: não foi possível inicializar GoldPixService - ' . $e->getMessage());
            return;
        }

        foreach ($this->emprestimo->parcelas as $parcela) {
            $this->criarCobrancaGoldpix($goldPixService, $parcela, $parcela->saldo ?? $parcela->valor, 'parcela_' . $parcela->id, $parcela->venc_real ? date('Y-m-d', strtotime($parcela->venc_real)) : null, function ($resp) use ($parcela) {
        ...
        if ($this->emprestimo->quitacao) {
            ...
            if ($valorQuitacao > 0) {
                $this->criarCobrancaGoldpix($goldPixService, $q, $valorQuitacao, 'quitacao_' . $q->id, null, function ($resp) use ($q) {
                    $q->identificador = $resp['transaction_id'] ?? $resp['txid'] ?? $q->identificador;
        ...
        if ($this->emprestimo->pagamentominimo) {
            ...
            $this->criarCobrancaGoldpix($goldPixService, $pm, $pm->valor, 'pagamento_minimo_' . $pm->id, null, function ($resp) use ($pm) {
                $pm->identificador = $resp['transaction_id'] ?? $resp['txid'] ?? $pm->identificador;
        ...
        if ($this->emprestimo->pagamentosaldopendente) {
            ...
            if ($psp->valor > 0) {
                $psp->save();
                $this->criarCobrancaGoldpix($goldPixService, $psp, $psp->valor, 'saldo_' . $psp->id, null, function ($resp) use ($psp) {
                    $psp->identificador = $resp['transaction_id'] ?? $resp['txid'] ?? $psp->identificador;
    ...
    /**
     * Chama GoldPixService criarCobranca e, em sucesso, aplica o callback para atualizar a entidade.
     */
    private function criarCobrancaGoldpix(GoldPixService $goldPixService, $entidade, float $valor, string $referenceId, ?string $dueDate, callable $onSuccess): void
    {
        $cliente = $this->emprestimo->client;
        ...
        while ($tentativas < $maxTentativas) {
            try {
                ...
                Log::channel('goldpix')->error('ProcessarPixGoldpixJob cobrança: ' . $e->getMessage(), [
...
";
    ...
    public function envioMensagemVideoYoutube($parcela): void
    {
        try {
            ...
            Log::channel('goldpix')->warning('ProcessarPixGoldpixJob envio vídeo: ' . $th->getMessage());
        }
    }

    public function failed(\Exception $exception): void
    {
        Log::channel('goldpix')->error('ProcessarPixGoldpixJob falhou: ' . $exception->getMessage());
    }
}
