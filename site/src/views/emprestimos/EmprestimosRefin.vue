...
export default {
    ...
    methods: {
        ...
        gerarParcelas() {
            ...
            const dataLanc = new Date(this.emprestimo.dt_lancamento);

            const dataInicial = new Date(this.emprestimo.dt_lancamento);
        ...
        getFeriados() {
            console.log('aqui aqui', this.address);
            this.emprestimo.valor = this.address?.valor + 100;
            this.emprestimo.pendente = this.convertStringToFloat(this.parcela?.total_pendente);
...
<template>
    ...
    <div class="grid flex flex-wrap mb-3 px-4 pt-2">
        <div class="col-12 px-0 py-0 text-right">
            <Button label="Renovar 80%" class="p-button-sm bg-roxo" :icon="icons.PLUS" @click="visibleRight = true" />
        </div>
    </div>
    <Sidebar v-model:visible="visibleRight" :baseZIndex="1000" position="right">
        <h1 style="font-weight: normal">Renovar 80%</h1>
        <div class="col-12">
            <div class="p-fluid formgrid grid">
                <div class="field col-12 md:col-12">
                    <label for="firstname2">Data Lançamento</label>
                    <Calendar dateFormat="dd/mm/yy" v-tooltip.left="'Selecione a data de Inicio'" v-model="emprestimo.dt_lancamento" showIcon :showOnFocus="false" class="" disabled="true"/>
                </div>
            </div>
            <div class="p-fluid formgrid grid">
                <div class="field col-12 md:col-12">
                    <label for="zip">Valor Liberado</label>
                    <InputNumber id="inputnumber" :modelValue="emprestimo?.valor" v-model="emprestimo.valor" :mode="'currency'" :currency="'BRL'" :locale="'pt-BR'" :precision="2" disabled class="w-full p-inputtext-sm"> </InputNumber>
                </div>
            </div>
            <div class="p-fluid formgrid grid">
                <div class="field col-12 md:col-12">
                    <label for="zip">Saldo Pendente a Descontar</label>
                    <InputNumber id="inputnumber" :modelValue="emprestimo?.pendente" v-model="emprestimo.valor" :mode="'currency'" :currency="'BRL'" :locale="'pt-BR'" :precision="2" disabled class="w-full p-inputtext-sm"> </InputNumber>
                </div>
            </div>
            <div class="p-fluid formgrid grid">
                <div class="field col-12 md:col-12">
