    ...
    methods: {
        ...
        getPermissions() {
            ...
            this.permissionsService
                .getAll()
                .then((response) => {
                    const per = response.data.data.flatMap((group) => ({
                        name: group.name,
                        id: group.id
                    }));
            ...
            if (this.route.params?.id) {
                ...
                this.usuarioService
                    ...
                    .then((response) => {
                        ...
                        const filteredData = this.usuario.permissao.filter((item) => item.company_id === this.$store?.getters?.isCompany.id);

                        let filtered = filteredData.flatMap((group) => ({
                            name: group.name,
                            id: group.id
                        }));

                        if (filteredData) {
                            this.selectedPermissao = filtered[0];
                        }
                    })
                    .catch((error) => {
                        this.toast.add({
        ...
        save() {
			...
			console.log('aa', this.selectedPermissao);

			if(this.selectedPermissao.name == undefined) {
				this.toast.add({
                    severity: ToastSeverity.ERROR,
            ...
            <div class="col-12">
                <div class="p-fluid formgrid grid">
                    ...
                    <div v-if="!this.route.params?.id" class="field col-12 md:col-3">
                        ...
                        <InputText id="firstname2" :modelValue="usuario?.login" v-model="usuario.login" type="text" />
                    </div>
                    <div class="field col-12 md:col-3">
                        <label for="firstname2">Nome Completo</label>
                        <InputText id="firstname2" :modelValue="usuario?.nome_completo" v-model="usuario.nome_completo" type="text" />
                    </div>
                    <div class="field col-12 md:col-3">
                    ...
                    <div class="field col-12 md:col-3">
                        ...
                        <InputMask id="inputmask" :modelValue="usuario?.data_nascimento" v-model="usuario.data_nascimento" mask="99/99/9999"></InputMask>
                    </div>
                    <div class="field col-12 md:col-3">
                        <label for="state">Telefone</label>
                        <InputMask id="inputmask" :modelValue="usuario?.telefone_celular" v-model="usuario.telefone_celular" mask="(99) 9999-9999"></InputMask>
                    </div>

                    <div class="field col-12 md:col-3">
                        <label for="state">CPF</label>
                        <InputMask id="inputmask" :modelValue="usuario?.cpf" v-model="usuario.cpf" mask="999.999.999-99"> </InputMask>
                    </div>
                    <div class="field col-12 md:col-3">
