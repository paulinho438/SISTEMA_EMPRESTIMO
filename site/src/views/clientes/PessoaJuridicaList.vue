	...
	methods: {
		initFilters() {
			this.filters = {
				...
				nome_fantasia: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
			...
			this.clientService
				...
				.catch((error) => {
					this.toast.add({
						...
						detail: error?.message || 'Erro ao carregar empresas',
						life: 3000
					});
				})
				.finally(() => {
		...
		editCliente(id) {
			if (undefined === id) this.router.push({ name: 'pjAdd' });
			else this.router.push({ name: 'pjEdit', params: { id } });
		},
		deleteCliente(id) {
			this.loading = true;
			this.clientService
				...
				.then((e) => {
					this.toast.add({
						...
						detail: e?.data?.message || 'Empresa excluída com sucesso',
						life: 3000
					});
					this.getClientes();
				})
		...
		<div class="col-12">
			<div class="grid flex flex-wrap mb-3 px-4 pt-2">
				<div class="col-8 px-0 py-0">
					<h5 class="px-0 py-0 align-self-center m-2"><i :class="icons.BUILDING"></i> Pessoas Jurídicas (Empresas)</h5>
				</div>
				<div class="col-4 px-0 py-0 text-right">
					<Button v-if="permissionsService.hasPermissions('view_clientes_create')" label="Nova Empresa" class="p-button-outlined p-button-secondary p-button-sm" :icon="icons.PLUS" @click.prevent="editCliente()" />
				</div>
			</div>
			<div class="col-12">
				<div class="card">
					<DataTable
						...
						:globalFilterFields="['razao_social', 'nome_fantasia', 'cnpj', 'telefone_celular_1']"
					>
						<template #header>
						...
						<Column field="razao_social" header="Razão Social" style="min-width: 14rem">
							<template #body="{ data }">
								{{ data.razao_social || data.nome_completo }}
							</template>
						</Column>
						<Column field="nome_fantasia" header="Nome Fantasia" style="min-width: 12rem">
							<template #body="{ data }">
								{{ data.nome_fantasia || '—' }}
							</template>
						</Column>
